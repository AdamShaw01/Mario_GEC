...
bool Texture2D::LoadFromFile(std::string path)
{
	...
	{
		std::cout << "Unable to create texture from the surface. Error: " << SDL_GetError();
	}
...
void Texture2D::Render(Vector2D new_position, SDL_RendererFlip flip, double angle)
{
	...
	SDL_Rect tenderLocation = { new_position.x, new_position.y, m_width, m_height };
	//render to screen
	SDL_RenderCopyEx(m_renderer, m_texture, nullptr, &tenderLocation, angle, nullptr, flip);
}
